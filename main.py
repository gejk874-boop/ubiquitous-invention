        ...
        try:
            with DatabaseConnection.get_connection() as conn:
                ...
                if fetch_one:
                    ...
                    result = cursor.lastrowid  # ВАЖНО: возвращаем ID последней вставленной записи
                    conn.commit()
                else:
...
def add_bot_user(user_id, username, first_name, last_name):
    ...
    try:
        query = '''
            INSERT OR REPLACE INTO bot_users (user_id, username, first_name, last_name, joined_date)
...
def add_report(reporter_id: int, target_username: str, status: str, comment: str, proof_photo: str = None) -> Optional[int]:
    ...
    try:
        query = '''
            ...
            VALUES (?, ?, ?, ?, ?)
        '''
        # ИСПРАВЛЕНО: используем return_lastrowid=True чтобы получить ID вставленной записи
