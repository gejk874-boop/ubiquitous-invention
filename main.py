...
def init_db():
    ...
    try:
        ...
        # НЕ УДАЛЯЕМ БД, пробуем восстановить
        try:
            if os.path.exists(DB_PATH):
                backup_name = f"backup_error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
                os.rename(DB_PATH, os.path.join(BACKUP_PATH, backup_name))
...
def create_backup():
    ...
    try:
        ...
        backup_name = os.path.join(BACKUP_PATH, f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db")
...
def block_user(user_id: int, username: str, reason: str, blocked_by: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        if user_id in is_user_blocked.cache:
            del is_user_blocked.cache[user_id]
...
async def cmd_get_db(message: types.Message):
    ...
    try:
        ...
        if create_backup():
            await message.answer("✅ Создан бэкап БД")
