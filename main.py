...
def check_spam(user_id):
    ...
    if len(spam_protection[user_id]) >= 10:  # больше 10 сообщений в минуту
...
def add_bot_user(user_id, username, first_name, last_name):
    """Добавляет пользователя бота в базу"""
    try:
        conn = sqlite3.connect('reports.db', check_same_thread=False)
        cursor = conn.cursor()
        cursor.execute('''
...
class UserState:
    ...
    WAITING_CHECK_USERNAME = "waiting_check_username"
...
async def handle_messages(client: Client, message: Message):
    try:
        ...
        if text == "📝 Жалоба":
            ...
            user_states[user_id] = {
                'state': UserState.WAITING_CHECK_USERNAME,
                'timestamp': datetime.now()
            }
            await message.reply(
...
- Ложные жалобы могут привести к блокировке
        ...
        if user_id in user_states:
            ...
            if state == UserState.WAITING_CHECK_USERNAME:
                if text.startswith('@'):
                    text = text[1:]
            ...
            elif state == UserState.WAITING_FOR_USERNAME:
                if text.startswith('@'):
                    text = text[1:]
                ...
                # Проверка на жалобу на самого себя
