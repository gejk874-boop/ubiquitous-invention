...
def init_db():
    ...
    try:
        ...
        # Если БД повреждена, удаляем и пробуем снова
        if os.path.exists('reports.db'):
            try:
                os.remove('reports.db')
                logger.info("🗑️ Удален поврежденный файл БД")
                # Рекурсивно вызываем снова
                init_db()
            except Exception as del_error:
                logger.error(f"❌ Не удалось удалить поврежденную БД: {del_error}")
...
def get_all_users_from_reports():
    """Получает всех уникальных пользователей из всех таблиц"""
    try:
        conn = sqlite3.connect('reports.db')
        cursor = conn.cursor()
        ...
        # 1. Пользователи из bot_users
        cursor.execute('SELECT DISTINCT user_id FROM bot_users')
        for row in cursor.fetchall():
            all_user_ids.add(row[0])
        
        # 2. Репортеры из reports
        cursor.execute('SELECT DISTINCT reporter_id FROM reports')
        for row in cursor.fetchall():
            all_user_ids.add(row[0])
        
        # 3. Модераторы из reports
        cursor.execute('SELECT DISTINCT moderator_id FROM reports WHERE moderator_id IS NOT NULL')
        for row in cursor.fetchall():
            all_user_ids.add(row[0])
        
        # 4. Заблокированные пользователи
        cursor.execute('SELECT DISTINCT user_id FROM blocked_users')
        for row in cursor.fetchall():
            all_user_ids.add(row[0])
        
        # 5. Пользователи, которые блокировали других
        cursor.execute('SELECT DISTINCT blocked_by FROM blocked_users WHERE blocked_by IS NOT NULL')
        for row in cursor.fetchall():
            all_user_ids.add(row[0])
        
        conn.close()
        
        logger.info(f"📊 Найдено {len(all_user_ids)} уникальных пользователей для рассылки")
...
# === ФУНКЦИЯ ОТПРАВКИ УВЕДОМЛЕНИЙ ОБ ОБНОВЛЕНИИ ===
async def send_update_notification():
    """Отправляет уведомление об обновлении всем пользователям бота"""
    # Получаем всех пользователей из всех источников
    all_user_ids = get_all_users_from_reports()
    ...
    logger.info(f"📢 Уведомления об обновлении отправлены: ✅ {success_count} успешно, ❌ {failed_count} с ошибками")
...
ℹ️ **Помощь** - получите справку по работе бота
    ...
    try:
        # Пробуем разные возможные пути к изображению
        image_paths = [
        ...
        for path in image_paths:
            if os.path.exists(path):
                ...
                logger.info(f"✅ Найдено изображение по пути: {path}")
                break
        ...
        if photo:
            ...
            # Если локальный файл не найден, попробуем загрузить из GitHub
...
# === ОБРАБОТЧИК УВЕДОМЛЕНИЯ ОБ ОБНОВЛЕНИИ ===
...
async def handle_admin_update_notify(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        ...
        "Это отправит сообщение ВСЕМ пользователям бота:\n"
        "• Об завершении обновления\n" 
        "• С просьбой перезапустить бота командой /start\n\n"
        "Продолжить?",
...
async def handle_confirm_update_notify(callback: types.CallbackQuery):
    ...
    success_count, failed_count = await send_update_notification()
    
    result_message = (
        f"📢 **Рассылка завершена!**\n\n"
        f"✅ Успешно отправлено: {success_count}\n"
        f"❌ С ошибками: {failed_count}\n\n"
        f"Все пользователи получили уведомление об обновлении."
    )
...
async def handle_reject_report(callback: types.CallbackQuery):
    ...
    if reporter_id:
        ...
        # Редактируем сообщение с жалобу
        try:
            await callback.message.edit_caption(
                f"❌ **Жалоба отклонена**\n\n"
                f"👁 Модератор: {callback.from_user.id}",
...
async def main():
    ...
    # ВАЖНО: Инициализируем БД только один раз при запуске
    init_db()
