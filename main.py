...
# === КОНФИГУРАЦИЯ ===
BOT_TOKEN = "8485410261:AAH_Enawu7wayql2NeO4FtDZ6Bv8EDp5LO4"
...
async def handle_admin_show_reports(callback: types.CallbackQuery):
    ...
    if not reports:
        ...
        for report in reports[:5]:  # Показываем первые 5 жалоб
...
async def handle_approve_report(callback: types.CallbackQuery):
    ...
    if reporter_id:
        try:
            ...
            pass
        await callback.message.edit_text(f"✅ Жалоба #{report_id} на @{target_username} одобрена")
    else:
        await callback.message.edit_text("❌ Жалоба уже обработана")
    await callback.answer()
