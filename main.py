...
from hydrogram.types import Message, ReplyKeyboardMarkup, KeyboardButton, InlineKeyboardMarkup, InlineKeyboardButton, ReplyKeyboardRemove
...
# Функция для получения клавиатуры пользователя (с кнопкой админа для админов)
...
def init_db():
    try:
        conn = sqlite3.connect('/tmp/reports.db', check_same_thread=False)
        cursor = conn.cursor()
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_blocked_username ON blocked_users(username)')
...
# ===== БЛОКИРОВКА ПО ЮЗЕРНЕЙМУ =====
def block_user_by_username(username, reason, blocked_by):
    try:
        conn = sqlite3.connect('/tmp/reports.db', check_same_thread=False)
        cursor = conn.cursor()
        
        # Проверяем, не заблокирован ли уже
        cursor.execute('SELECT id FROM blocked_users WHERE username = ?', (username.lower(),))
        if cursor.fetchone():
            conn.close()
            return False, "❌ Пользователь с этим юзернеймом уже заблокирован"
            
        cursor.execute('''
            INSERT INTO blocked_users (username, reason, blocked_by)
            VALUES (?, ?, ?)
        ''', (username.lower(), reason, blocked_by))
        conn.commit()
        conn.close()
        logger.info(f"✅ Юзернейм @{username} заблокирован")
        return True, f"✅ Юзернейм @{username} успешно заблокирован"
    except Exception as e:
        logger.error(f"❌ Ошибка блокировки по юзернейму: {e}")
...
def unblock_user_by_username(username):
    try:
        conn = sqlite3.connect('/tmp/reports.db', check_same_thread=False)
        cursor = conn.cursor()
        cursor.execute('DELETE FROM blocked_users WHERE username = ?', (username.lower(),))
        rows_affected = cursor.rowcount
        conn.commit()
        conn.close()
        
        if rows_affected > 0:
            logger.info(f"✅ Юзернейм @{username} разблокирован")
            return True, f"✅ Юзернейм @{username} успешно разблокирован"
        else:
            return False, "❌ Пользователь с таким юзернеймом не найден в списке заблокированных"
    except Exception as e:
        logger.error(f"❌ Ошибка разблокировки по юзернейму: {e}")
...
def is_username_blocked(username):
    try:
        ...
        logger.error(f"❌ Ошибка проверки блокировки по юзернейму: {e}")
        return False
...
class AdminState:
    ...
    WAITING_BLOCK_USERNAME = "waiting_block_username"
    WAITING_UNBLOCK_USERNAME = "waiting_unblock_username"
...
- Ложные жалобы могут привести к блокировке
        """
        await message.reply(help_text, reply_markup=get_user_keyboard(message.from_user.id))
        
    except Exception as e:
        logger.error(f"❌ Ошибка в handle_help_button: {e}")
        await message.reply("❌ Произошла ошибка при показе справки")

async def handle_admin_button(client: Client, message: Message):
    try:
        user_id = message.from_user.id
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🚫 Заблокировать по ID", callback_data="admin_block")],
            [InlineKeyboardButton("✅ Разблокировать по ID", callback_data="admin_unblock")],
            [InlineKeyboardButton("🚫 Заблокировать по юзернейму", callback_data="admin_block_username")],
            [InlineKeyboardButton("✅ Разблокировать по юзернейму", callback_data="admin_unblock_username")],
...
ℹ️ **Помощь** - получить справку по работе бота
        """
        await message.reply(welcome_text, reply_markup=get_user_keyboard(user_id))
...
async def cmd_admin(client: Client, message: Message):
    await handle_admin_button(client, message)
...
async def handle_messages(client: Client, message: Message):
    try:
        ...
        elif text == "🛠 Админ" and user_id in ADMIN_IDS:
            await handle_admin_button(client, message)
            return
        ...
        if user_id in user_states and user_states[user_id]['state'] == "waiting_check_username":
            ...
            # Проверяем, не заблокирован ли юзернейм
            if is_username_blocked(text):
                await message.reply(f"❌ Пользователь @{text} заблокирован в системе.", reply_markup=get_user_keyboard(user_id))
                del user_states[user_id]
                return
            ...
            if not reports:
                await message.reply(f"ℹ️ По пользователю @{text} информации нет", reply_markup=get_user_keyboard(user_id))
            else:
                statuses = set()
                comments = []
        ...
        if user_id in user_states:
            ...
            if state == UserState.WAITING_FOR_USERNAME:
                ...
                # Проверяем, не заблокирован ли юзернейм
                if is_username_blocked(text):
                    await message.reply(f"❌ Нельзя подать жалобу на заблокированного пользователя @{text}", reply_markup=get_user_keyboard(user_id))
                    del user_states[user_id]
                    return
        ...
        # Обработка админских состояний
            ...
            if state == AdminState.WAITING_BLOCK_USER:
                ...
                await message.reply(result_msg, reply_markup=get_user_keyboard(user_id))
                ...
                try:
                    ...
                    await message.reply("❌ Введите числовой ID пользователя", reply_markup=get_user_keyboard(user_id))
            ...
            elif state == AdminState.WAITING_BLOCK_USERNAME:
                if text.startswith('@'):
                    text = text[1:]
                ...
                success, result_msg = block_user_by_username(text, "Блокировка по юзернейму", user_id)
                await message.reply(result_msg, reply_markup=get_user_keyboard(user_id))
                del user_states[user_id]

            elif state == AdminState.WAITING_UNBLOCK_USERNAME:
                if text.startswith('@'):
                    text = text[1:]
...
async def save_report(client: Client, user_id: int, status: str, message: Message):
    try:
        ...
        if report_id:
            ...
            await message.reply("❌ Ошибка сохранения заявки", reply_markup=get_user_keyboard(user_id))
        ...
        await message.reply("❌ Ошибка при сохранении заявки", reply_markup=get_user_keyboard(user_id))
...
async def handle_callbacks(client: Client, callback_query):
    try:
        ...
        elif data == "admin_block_username":
            user_states[user_id] = {
                'state': AdminState.WAITING_BLOCK_USERNAME,
                'timestamp': datetime.now()
            }
            await callback_query.message.edit_text(
                "Введите юзернейм пользователя для блокировки (без @):"
            )
            await callback_query.answer()

        elif data == "admin_unblock_username":
            user_states[user_id] = {
                'state': AdminState.WAITING_UNBLOCK_USERNAME,
                'timestamp': datetime.now()
            }
            await callback_query.message.edit_text(
                "Введите юзернейм пользователя для разблокировки (без @):"
            )
            await callback_query.answer()
