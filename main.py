...
➕ **Добавить в чат** - добавить бота в группу или канал
    ...
    try:
        ...
        if photo:
            await message.answer_photo(photo=photo, caption=welcome_text, reply_markup=get_user_keyboard(user_id))
        else:
            github_url = "https://raw.githubusercontent.com/gejk874-boop/ubiquitous-invention/main/Lumii_20251122_105626106.jpg"
            await message.answer_photo(photo=github_url, caption=welcome_text, reply_markup=get_user_keyboard(user_id))
    ...
    for user_id in all_user_ids:
        try:
            if not is_user_blocked(user_id):
                update_message = "🔄 <b>ОБНОВЛЕНИЕ БОТА ЗАВЕРШЕНО!</b>\n\n✅ Были добавлены новые функции\n📲 <b>Перезапустите бота командой</b> /start"
...
async def handle_approve_report(callback: types.CallbackQuery):
    ...
    if reporter_id:
        ...
        try:
            await callback.message.edit_caption(f"✅ **Жалоба одобрена**\n\n👤 Пользователь: @{target_username}\n👁 Модератор: {callback.from_user.id}", reply_markup=None)
        except:
            try:
                await callback.message.edit_text(f"✅ **Жалоба одобрена**\n\n👤 Пользователь: @{target_username}\n👁 Модератор: {callback.from_user.id}", reply_markup=None)
            except:
                pass
    else:
...
async def handle_reject_report(callback: types.CallbackQuery):
    ...
    if reporter_id:
        ...
        try:
            ...
            try:
                await callback.message.edit_text(f"❌ **Жалоба отклонена**\n\n👁 Модератор: {callback.from_user.id}", reply_markup=None)
            except:
                pass
    else:
        await callback.answer("❌ Жалоба уже обработана", show_alert=True)
