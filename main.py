...
# Функция для получения клавиатуры пользователя
...
async def handle_admin_button(client: Client, message: Message):
    try:
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🔙 В главное меню", callback_data="admin_back_to_main")]
        ])
...
@app.on_message(filters.command("complaint"))
async def cmd_complaint(client: Client, message: Message):
    if is_user_blocked(message.from_user.id):
        await message.reply("❌ Вы заблокированы в системе.")
        return
...
async def handle_messages(client: Client, message: Message):
    try:
        ...
        # ПРОВЕРКА БЛОКИРОВКИ В НАЧАЛЕ ОБРАБОТКИ СООБЩЕНИЙ
        if is_user_blocked(user_id):
            await message.reply("❌ Вы заблокированы в системе.")
            return
        ...
        # Обработка кнопки "Назад" - УЛУЧШЕННАЯ ЛОГИКА
        if text == "🔙 Назад":
            if user_id in user_states:
                state = user_states[user_id].get('state', '')
                # Если админ в админ-состоянии - возврат в админ-панель
                if user_id in ADMIN_IDS and any(admin_state in str(state) for admin_state in ['waiting_block', 'waiting_unblock', 'waiting_delete', 'waiting_announcement']):
                    await handle_admin_button(client, message)
                else:
        ...
        if user_id in user_states:
            ...
            if state == UserState.WAITING_FOR_USERNAME:
                ...
                if not text or len(text) < 5:
                    await message.reply("❌ Комментарий слишком короткий (минимум 5 символов). Попробуйте снова:", reply_markup=back_keyboard)
                    return
                
                if len(text) > 500:
                    await message.reply("❌ Комментарий слишком длинный (максимум 500 символов). Попробуйте снова:", reply_markup=back_keyboard)
                    return
                ...
                if text.lower() == 'пропустить':
                    ...
                    await message.reply("❌ Отправьте фото как доказательство или напишите 'пропустить'", reply_markup=back_keyboard)
        ...
        # Обработка админских состояний - ИСПРАВЛЕННАЯ ЛОГИКА
            ...
            if state == AdminState.WAITING_BLOCK_USER:
                try:
                    target_id = int(text)
                    # Получаем информацию о пользователе для блокировки
                    try:
                    ...
                    user_data[user_id]['block_target_username'] = username
                    await message.reply(
                    ...
                    await message.reply("❌ Введите числовой ID пользователя:", reply_markup=back_keyboard)
                
            elif state == AdminState.WAITING_BLOCK_REASON:
                target_id = user_data[user_id]['block_target_id']
                target_username = user_data[user_id]['block_target_username']
                reason = text
                
                success, result_msg = block_user(target_id, target_username, reason, user_id)
                # ОТПРАВКА СООБЩЕНИЯ АДМИНУ О РЕЗУЛЬТАТЕ
                ...
                # РАССЫЛКА ВСЕМ ПОЛЬЗОВАТЕЛЯМ - ИСПРАВЛЕННАЯ ЛОГИКА
