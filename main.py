...
def get_all_users_for_broadcast():
    """Получает всех пользователей для рассылки из bot_users"""
    try:
        conn = sqlite3.connect('reports.db', check_same_thread=False)
        cursor = conn.cursor()
        
        # Получаем ВСЕХ пользователей из bot_users
        cursor.execute('SELECT DISTINCT user_id FROM bot_users WHERE user_id IS NOT NULL')
    ...
    for index, user_id in enumerate(all_user_ids, 1):
        try:
            if not is_user_blocked(user_id):
                ...
                success_count += 1
                if index % 10 == 0:  # Логируем каждые 10 отправок
                    logger.info(f"📤 Отправлено {index}/{len(all_user_ids)} пользователям")
            else:
                logger.info(f"⏭️ Пропущен заблокированный в системе пользователь: {user_id}")
...
def get_user_keyboard(user_id):
    ...
    if user_id in ADMIN_IDS:
        ...
        keyboard.append([KeyboardButton(text="📥 Скачать БД")])
...
# === КОМАНДА ДЛЯ СТАТИСТИКИ ПОЛЬЗОВАТЕЛЕЙ ===
...
async def cmd_users(message: types.Message):
    ...
    cursor.execute('SELECT user_id, username, joined_date FROM bot_users ORDER BY joined_date DESC LIMIT 10')
    ...
    response = [
        ...
        f"👥 Всего пользователей в bot_users: <b>{total_users}</b>",
        f"🚫 Заблокировано в системе: <b>{blocked_users}</b>",
        f"📢 Для рассылки доступно: <b>{len(users_for_broadcast)}</b>",
        f"",
