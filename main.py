...
def get_all_users_from_reports():
    """Получает всех уникальных пользователей из таблицы reports"""
    try:
        conn = sqlite3.connect('reports.db')
        cursor = conn.cursor()
        cursor.execute('SELECT DISTINCT reporter_id FROM reports')
        users_from_reports = [row[0] for row in cursor.fetchall()]
        
        cursor.execute('SELECT DISTINCT user_id FROM bot_users')
        users_from_bot = [row[0] for row in cursor.fetchall()]
        
        # Объединяем и убираем дубликаты
        all_users = list(set(users_from_reports + users_from_bot))
        conn.close()
...
async def send_update_notification():
    ...
    users_from_db = get_all_bot_users()
    user_ids_from_reports = get_all_users_from_reports()
    
    # Собираем все ID пользователей
    all_user_ids = set()
    
    # Добавляем пользователей из bot_users
    for user_id, username in users_from_db:
        all_user_ids.add(user_id)
    
    # Добавляем пользователей из reports
    for user_id in user_ids_from_reports:
        all_user_ids.add(user_id)
