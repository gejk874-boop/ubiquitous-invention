...
# Устанавливаем более стабильные параметры бота
bot = Bot(
    token=BOT_TOKEN, 
    timeout=30,
...
class DatabaseConnection:
    ...
    def execute_query(query: str, params: tuple = (), fetch_one: bool = False, 
        ...
        try:
            ...
            # Попробуем повторно инициализировать БД при ошибке таблиц
            if "no such table" in str(e):
                logger.warning("🔄 Попытка восстановить таблицы БД...")
                init_db()
            raise
        except Exception as e:
            logger.error(f"❌ Неожиданная ошибка в запросе: {e}")
            raise
...
def get_user_keyboard(user_id):
    """Клавиатура для пользователя с кнопкой Добавить бота"""
    keyboard = [
        [KeyboardButton(text="📝 Жалоба"), KeyboardButton(text="🔍 Проверить")],
        [KeyboardButton(text="➕ Добавить бота"), KeyboardButton(text="ℹ️ Помощь")]
    ]
...
async def cmd_start(message: types.Message):
    ...
    welcome_text = """
🎯 <b>Добро пожаловать в бот проверки пользователей!</b>

📝 <b>Жалоба</b> - сообщить о ненадежном пользователе
🔍 <b>Проверить</b> - узнайте информацию о пользователе  
➕ <b>Добавить бота</b> - добавить бота в чат/группу
ℹ️ <b>Помощь</b> - получите справку по работе бота
    """
...
async def cmd_users(message: types.Message):
    ...
    try:
        ...
        if result:
            total_users, blocked_users, total_reports, approved_reports, pending_reports, today_users = result
...
async def cmd_get_db(message: types.Message):
    ...
    try:
        ...
        if file_size == 0:
            await message.answer("❌ Файл базы данных пустой")
            return
...
# ============================================
# КОМАНДА /CHECK ДЛЯ ГРУПП
# ============================================
@dp.message(Command("check"))
async def cmd_check(message: types.Message):
    """Обработка команды /check в группах"""
    chat_type = message.chat.type
    
    # Проверяем, что команда используется в группе или супергруппе
    ...
    try:
        ...
        if chat_member.status not in ["administrator", "member"]:
            await message.answer("❌ Бот должен быть участником группы для использования этой команды.")
            return
    except Exception as e:
        logger.error(f"❌ Ошибка проверки статуса бота в группе: {e}")
        await message.answer("❌ Ошибка проверки статуса бота.")
        return
    
    # Получаем username из команды
    args = message.text.split()
    if len(args) < 2:
        await message.answer("❌ Используйте: /check @username")
        return
    ...
    if not reports:
        await message.answer(f"ℹ️ Информация о @{username} не найдена в базе данных.")
    else:
        statuses = set()
        comments = []
        ...
        response = [
            ...
            f"🏷 <b>Статусы:</b> {', '.join(sorted(statuses))}",
            f"📝 <b>Комментарии:</b>",
            *comments[:3],
            f"📊 <b>Всего жалоб:</b> {len(reports)}"
        ]
        
        await message.answer("\n".join(response))

# ============================================
# ОБРАБОТЧИК КНОПКИ "➕ ДОБАВИТЬ БОТА"
# ============================================
@dp.message(F.text == "➕ Добавить бота")
async def handle_add_bot(message: types.Message):
    """Обработчик кнопки добавления бота"""
    user_id = message.from_user.id
    ...
    bot_username = "BazaSkama_bot"  # Ваш username бота
    bot_name = (await bot.me()).full_name
    
    add_bot_message = f"""
🤖 <b>{bot_name}</b>

✅ Добавьте меня в ваш чат или группу для проверки пользователей!

📋 <b>Как добавить:</b>
1. Откройте меню управления группой
2. Выберите "Добавить участников"
3. Введите мой username: @{bot_username}

🎯 <b>Функции в группах:</b>
• Проверка пользователей командой <code>/check @username</code>
• Модерация жалоб
...
📲 <b>Также доступно:</b> @{bot_username}
    """
    
    # Создаем inline-кнопку для быстрого добавления
...
async def bot_added_to_chat(update: ChatMemberUpdated):
    ...
    logger.info(f"🤖 Бот добавлен в {chat_type}: {chat_title} (ID: {chat_id})")
    
    # Отправляем приветственное сообщение
    if chat_type in ["group", "supergroup"]:
        welcome_message = f"""
👋 <b>Приветствую в группе {chat_title}!</b>

🤖 Я — бот для проверки пользователей.

📋 <b>Доступные команды:</b>
• <code>/check @username</code> — проверить информацию о пользователе
• <code>/start</code> — личное меню бота

💡 <b>Для полного функционала:</b>
1. Дайте боту права администратора
2. Используйте /check для проверки пользователей
...
📊 База данных регулярно обновляется.
        ...
        try:
            ...
            logger.error(f"❌ Не удалось отправить приветствие в группу {chat_id}: {e}")
    elif chat_type == "channel":
        try:
            await bot.send_message(
                chat_id=chat_id,
                text="📢 Бот добавлен в канал. Используйте /start для личного меню."
            )
        except Exception as e:
            logger.error(f"❌ Не удалось отправить сообщение в канал {chat_id}: {e}")
...
async def process_comment(message: types.Message, state: FSMContext):
    ...
    await message.answer("📎 <b>Пришлите скриншот или нажмите 'Пропустить':</b>", reply_markup=proof_keyboard)
...
async def process_status(message: types.Message, state: FSMContext):
    ...
    if status == "другое":
        ...
        await message.answer("✏️ <b>Введите свой вариант статуса:</b>", reply_markup=back_keyboard)
        return
...
async def save_report(message: types.Message, state: FSMContext, status: str):
    ...
    if report_id:
        ...
        admin_text = (f"🆕 Новая жалоба #{report_id}\n\n"
                     f"👤 <b>От кого:</b> {reporter_username}\n"
                     f"🚨 <b>На кого:</b> @{data['target_username']}\n"
        ...
        await message.answer("✅ <b>Жалоба отправлена на модерацию!</b>", reply_markup=get_user_keyboard(message.from_user.id))
    else:
...
async def handle_check(message: types.Message, state: FSMContext):
    ...
    await message.answer("🔍 <b>Введите юзернейм для проверки:</b>", reply_markup=back_keyboard)
...
📝 <b>Жалоба</b> - нажмите кнопку и следуйте инструкциям
🔍 <b>Проверить</b> - узнайте информацию о пользователе
➕ <b>Добавить бота</b> - добавьте бота в группу для проверки участников

⚠️ <b>Внимание:</b> 
- Максимум 5 жалоб в час
- Жалобы проходят модерацию
- В группах используйте команду /check @username
    """
...
async def handle_confirm_update_notify(callback: types.CallbackQuery):
    ...
    await callback.message.answer(result_message, reply_markup=get_user_keyboard(callback.from_user.id))
    await callback.answer()
...
async def handle_admin_show_reports(callback: types.CallbackQuery):
    ...
    for report in pending_reports:
        ...
        report_text = (f"🆕 Жалоба #{report_id}\n\n"
                      f"👤 <b>От кого:</b> ID {reporter_id}\n"
                      f"🚨 <b>На кого:</b> @{target_username}\n"
...
async def handle_admin_block_username(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("🚫 <b>Введите @username для блокировки:</b>\n(например: username)", reply_markup=back_keyboard)
...
async def process_admin_block_username(message: types.Message, state: FSMContext):
    ...
    await message.answer("📝 <b>Введите причину блокировки:</b>", reply_markup=back_keyboard)
...
async def process_admin_block_reason(message: types.Message, state: FSMContext):
    ...
    try:
        await bot.send_message(data['target_user_id'], f"🚫 <b>Вы заблокированы!</b>\nПричина: {reason}")
    except:
        pass
...
async def handle_admin_unblock_username(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("✅ <b>Введите @username для разблокировки:</b>\n(например: username)", reply_markup=back_keyboard)
...
async def handle_admin_announcement(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("📢 <b>Введите текст объявления:</b>", reply_markup=back_keyboard)
...
async def process_admin_announcement(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"📢 <b>Объявление отправлено {success_count} пользователям</b>", reply_markup=get_user_keyboard(message.from_user.id))
    await state.clear()
...
async def handle_admin_delete_user(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("🗑️ <b>Введите @username для удаления информации:</b>\n(например: username)", reply_markup=back_keyboard)
...
async def main():
    ...
    # Инициализируем БД
    if not init_db():
        logger.critical("❌ Не удалось инициализировать базу данных. Бот остановлен.")
        sys.exit(1)
    ...
    while restart_count < max_restarts:
        try:
            ...
            restart_count += 1
            logger.error(f"❌ Сетевая ошибка (попытка {restart_count}/{max_restarts}): {e}")
